...
#+ histogram
ggplot(d) +
  geom_histogram(aes(x = Lymphocytes, fill = disease_assay), bins = 30) +
...
d$Active <- d$Lymphocytes > root$root
table(d$disease_assay, d$Active)

ggplot(d) +
  geom_histogram(
    aes(x = Lymphocytes, fill = disease_assay),
    bins = 30
  ) +
  scale_fill_manual(
...
ggplot(d) +
  geom_histogram(aes(x = CD45pos.ov.Live, fill = disease_assay), bins = 30) +
  scale_fill_manual(values = meta_colors$disease_assay)
...
table(d$disease_assay, d$Active2)

ggplot(d) +
  geom_histogram(
    aes(x = CD45pos.ov.Live, fill = disease_assay),
    bins = 30
  ) +
  scale_fill_manual(
    values = meta_colors$disease_assay,
