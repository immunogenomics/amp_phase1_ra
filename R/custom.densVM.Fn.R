...
# 31 Jan 2017
#
# Objective: Custom peak-finding functions modified from densVM. Added option to
#            directly set bandwidth and skip search loop, and increased range of
#            kernel bandwidths searched. Finally, SVM object is returned by function.
################################################################################
require(e1071)

densVM_cluster <- function(ydata, xdata, sig_given=NULL) {
  y_range_x <- max(ydata[, 1]) - min(ydata[, 1])
  y_range_y <- max(ydata[, 2]) - min(ydata[, 2])
  y_range <- min(c(y_range_x, y_range_y))
  
  if (is.null(sig_given)) {
    ...
    opt_densityIMG <- peakFind(ydata, sig_given)
    peakdata <- data.frame(numpeaks = opt_densityIMG$Npeaks, sig_range = sig_given)
  }
  print(paste("Found", opt_densityIMG$Npeaks, "peaks"))
  output <- assign_pop(opt_densityIMG, ydata, xdata)
  clusters <- output$results
  svm <- output$svm
  print("Assigned all cells to clusters, done")
  
  return(list(peakdata = peakdata, clusters = clusters, svm = svm))
}
...
assign_pop <- function(opt_densityIMG, ydata, xdata) {
  ...
  for (i in 1:p_num) {
    ...
    dists <- (kronecker(matrix(1, p_num - 1, 1), matrix(p[i,], 1, 2)) - p[-i, ])^2
  ...
  return(list(results = clusterResults, svm = svm.obj))
}
